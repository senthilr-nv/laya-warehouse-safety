...
class LayaController:
    ...
    QUESTIONS = {
        "action": {
            "type": "choice",
            "instructions": "Which action should the warehouse robot take for the next tick?",
            "criteria": {
                "advance": "move one row toward the loading bay",
                "shift_left": "move one column left to avoid a hazard",
                "shift_right": "move one column right to avoid a hazard",
                "wait": "remain in place while traffic passes",
            },
        },
        "collision_risk": {
            "type": "score",
    ...
    def decide(self, observation: dict[str, Any]) -> Decision:
        ...
        return Decision(
            ...
            details={"answers": result["answers"], "usage": result["usage"]},
        )
